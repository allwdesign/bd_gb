...
-- Проверяю содержимое таблиц users и posts
SELECT * FROM users;
SELECT * FROM posts;

# Т.к. в таблице posts есть пользователь с user_id=7, а в таблице users нет такого пользователя
# При попытке добавить внешний ключ будет ошибка 1452.
DELETE FROM posts
...
-- Проверяю содержимое таблицы clients
SELECT * FROM clients;
