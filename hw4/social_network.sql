...
SELECT u.id, 
	u.firstname,
	    ...
	    SELECT media.filename 
            FROM media 
            WHERE media.id = (SELECT photo_id FROM profiles WHERE user_id = u.id)
	) AS avatarka
FROM users AS u;
...
SELECT m.filename
FROM media AS m
WHERE m.user_id = (SELECT u.id FROM users AS u WHERE u.email = 'arlo50@example.org') -- u.id = 1
AND m.filename REGEXP '\.(jpe?g|gif|png|bmp|tif?f|dib)';
...
WHERE m.user_id = (
		SELECT u.id 
                    FROM users AS u 
                    WHERE u.email = 'arlo50@example.org') -- u.id = 1
AND m.media_type_id = (
			SELECT mt.id 
                        FROM media_types AS mt 
                        WHERE mt.name_type = 'Photo'); -- mt.id = 1
...
-- выбрать всех пользователей, указав их id, имя и фамилию, город и аватарку
-- (используя JOIN)
SELECT u.id, u.firstname, u.lastname, p.hometown, m.filename AS avatarka
FROM users AS u
...
-- (используя JOIN)
SELECT media.id, media.filename, mt.name_type
FROM media
